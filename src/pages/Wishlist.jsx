...
function Wishlist({ goTo }) {
  ...
  const wishlistItems = t('wishlist.items', { returnObjects: true }) || []

  return (
    <>
      <div className="page-hero-img" style={{ height: '52vh' }}>
        <img src={cmsImages.wishlist_hero || '/img/kim-savana.svg'} alt="Wishlist Nova's Legacy" />
        <div className="page-hero-img-overlay" />
        <div className="page-hero-text">
          <span className="label label-light">{t('wishlist.sub_label')}</span>
          <h1>
            {t('wishlist.title_start')} <em>{t('wishlist.title_em')}</em>
          </h1>
        </div>
      </div>
      ...
      <div className="donate-page">
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '1rem 1.5rem' }}>
          ...
          <div className="rv" style={{ color: '#555', lineHeight: '1.7', fontWeight: 300, fontSize: '0.95rem', marginBottom: '2rem' }}>
            <p style={{ marginBottom: '1rem' }}>{t('wishlist.p1')}</p>
            <p style={{ marginBottom: '1.5rem' }}>{t('wishlist.p2')}</p>

            <div className="highlight" style={{ margin: '2rem 0' }}>
              <p style={{ margin: 0 }}>
                ◆ <strong>{t('wishlist.urgent_title')}</strong> {t('wishlist.urgent_text')}
              </p>
            </div>
          </div>
          ...
          <div className="rv" style={{ marginBottom: '2rem' }}>
            <h3 style={{ fontFamily: 'var(--serif)', fontSize: '1.6rem', color: 'var(--dark)', borderBottom: '2px solid #EDE5D8', paddingBottom: '0.5rem', marginBottom: '1.5rem' }}>
              {t('wishlist.need_start')} <em>{t('wishlist.need_em')}</em>
            </h3>
